...
class UserRoleFromCrudUSersCest
{
     ...
     * perform login with given email/password
     */
    private function loginWithRolePlayer(AcceptanceTester $I)
    {
        $email = 'player@icloud.com';
        $password = 'player';

        $I->amOnPage('/login');
        $I->expect('to successfully login as a ROLE_PLAYER user');
        $I->fillField('#inputEmail', $email);
        $I->fillField('#inputPassword', $password);
        $I->click('Login');
     ...
     * perform login with given email/password
     */
    private function loginWithRoleCoach(AcceptanceTester $I)
    {
        $email = 'coach@icloud.com';
        $password = 'coach';

        $I->amOnPage('/login');
        $I->expect('to successfully login as a ROLE_COACH user');
        $I->fillField('#inputEmail', $email);
        $I->fillField('#inputPassword', $password);
        $I->click('Login');
    ...
    private function clickAdminHomeLinkAndSeeSecrets(AcceptanceTester $I)
    {
        ...
        $I->see('welcome to the admin home page');
    }
    ...
    private function visitAdminSeeAccessDeniedMessage(AcceptanceTester $I)
    {
        ...
        $I->dontSee('welcome to the admin home page');
    }

    /**
     * @example(email="test1@icloud.com", password="test1", role="ROLE_USER")
     * @example(email="test2@icloud.com", password="test2", role="ROLE_ADMIN")
     */
    ...
    /**
     * @example(email="test1@icloud.com", password="test1", role="ROLE_USER")
     * @example(email="test2@icloud.ie", password="test2", role="ROLE_ADMIN")
     */
